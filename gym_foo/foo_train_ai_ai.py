...
if __name__ == "__main__":
    ...
    is_start = True
    agent_0 = QLearnAgent_V2(team=0, agent_model_path=agent_model_path_0)
    agent_1 = QLearnAgent(team=1, agent_model_path=agent_model_path_1)
    while True:
        if is_start:
        ...
        agent1_a0, agent1_action_list = agent_1.generate_action(state, score, detail_for_agent1)
        ...
        # train agent1
        # s1处的最大动作
        a1 = agent_1.greedy(agent1_s1)
        key1 = "%s_%s" % (agent1_s1, a1)
        key = "%s_%s" % (agent1_s0, agent1_a0)
        # 利用qlearning方法更新值函数
        agent_1.qfunc[key] = agent_1.qfunc.get(key, 0.0) + alpha * (
                    agent1_r0 + gamma * agent_1.qfunc.get(key1, 0.0) - agent_1.qfunc.get(key, 0.0))
