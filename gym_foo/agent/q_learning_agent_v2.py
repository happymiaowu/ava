...
class QLearnAgent_V2(Agent):
    ...
    def __init__(self, team: object, agent_model_path: object = None) -> object:
        super().__init__(team)
        self.qfunc = dict()
        self.valid_action = ['attack_tower',
                             'attack_hive',
                             'reinforce_tower',
                             'reinforce_hive',
                             'rally_tower',
    ...
    def generate_action(self, state, score, detail: BattleField):
        ...
        r, s = get_status_rewards_from_detail(detail)
        ql_action = self.epsilon_greedy(s, 0.2)

        for hive in detail.get_self_hives():

            # random_action = random.choice(self.valid_action)
            random_action = ql_action

            if random_action == 'attack_tower':
            ...
            if random_action == 'reinforce_tower':
                if self.is_march_num_full(hive):
                    continue
                target = random.choice(tower)
                if type(target) == Tower and target.who_occupied() != self._team:
        ...
        return ql_action, ActionList(team=self._team, action_list=action_list)
